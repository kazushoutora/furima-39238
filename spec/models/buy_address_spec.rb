  ...
  describe '購入記録の保存' do
    ...
    context '購入内容に問題がある場合' do
      ...
      it '郵便番号が『3桁ハイフン4桁』半角数字でないと購入できない' do
        ...
        @buy_address.valid?
        expect(@buy_address.errors.full_messages).to include('郵便番号は『3桁ハイフン4桁』半角数字で入力してください')
      end

      it '都道府県に「---」が選択されている場合は購入できない' do
        @buy_address.delivery_from_id = 1
        @buy_address.valid?
        expect(@buy_address.errors.full_messages).to include("発送元の地域を入力してください")
      end
      ...
      it '電話番号が9桁以下だと購入できないこと' do
        ...
        @buy_address.valid?
        expect(@buy_address.errors.full_messages).to include('電話番号は10桁以上11桁以内で入力してください')
      end
      ...
      it 'tokenが空では購入できない' do
        ...
        @buy_address.valid?
        expect(@buy_address.errors.full_messages).to include("カード情報を入力してください")
      end
    end
  end
end
